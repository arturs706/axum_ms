...
pub async fn upload_images(
    ...
    let token = auth.to_str().map_err(|_| CustomErrors::MissingCreds).unwrap();
    let authtoken = token.replace("Bearer ", "");
    let base_url = "http://0.0.0.0:10003/api/v1/property/uploadedimg/"; // Replace with your actual server URL
    ...
    match jwt_repo::validate_token(&authtoken).await {
        Ok(_) => {
            ...
            if let Err(error) = tfs::create_dir_all("./uploadedimg/").await {
                ...
                return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Error creating directory: {}", error)));
            }
            ...
            while let Some(mut field) = payload.next_field().await.map_err(|error| {
                ...
                let file_extension = match file_name.split('.').last() {
                    ...
                    None => {
                        ...
                        return Err((StatusCode::BAD_REQUEST, "No file extension found".to_string()));
                    }
                };
                ...
                let mut file = match File::create(&file_path) {
                    ...
                    Err(error) => {
                        ...
                        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Error opening file for writing: {}", error)));
                    }
                };
...
async fn handle_image_processing(destination: String) -> String {
    ...
    let new_file_path = format!("{}{}.webp", upload_dir, file_name_without_extension);

    uploaded_img
        .resize_exact(1920, 1080, FilterType::Gaussian)
        .save(&new_file_path).unwrap();
